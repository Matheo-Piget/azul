...
export class SummerPavilionEngine implements AzulGameEngine {
  initializeGame(players: string[]): GameState {
    ...
    let gameState: GameState = {
      ...
      bonusTiles: this.createBonusTiles(),
    };
  ...
  private createBonusTiles(): Tile[] {
    const bonusTiles: Tile[] = [];
    const colors = [...SUMMER_COLORS];

    // Mélanger les couleurs et prendre 10 tuiles
    const shuffledColors = shuffle([...colors, ...colors]); // Doubler pour avoir plus de choix
  ...
  applyMove(gameState: GameState, move: any): GameState {
    ...
    if (gameState.gamePhase === "drafting") {
      ...
      if (factoryId !== null) {
        ...
        const factoryIndex = newState.factories.findIndex(
          (f) => f.id === factoryId
        );
        ...
        if (color !== jokerColor) {
          const jokerTiles = factory.tiles.filter(
            (t) => t.color === jokerColor
          );
        ...
        const remainingTiles = factory.tiles.filter(
          (t) => !selectedTiles.includes(t)
        );

        // Ajoute les tuiles sélectionnées au joueur
        const existingIds = new Set(
          currentPlayer.board.collectedTiles?.map((t) => t.id) || []
        );
        const uniqueNewTiles = selectedTiles.filter(
        ...
        if (newState.firstPlayerToken === null) {
          ...
          const jokerInCenter = newState.center.find(
            (t) => t.color === jokerColor
          );
          ...
          const newScore = Math.max(
            1,
            currentPlayer.board.score - pointPenalty
          );
        ...
        // Si on ne sélectionne pas directement des jokers, il faut prendre une tuile joker si présente
        if (color !== jokerColor) {
          const jokerTiles = newState.center.filter(
            (t) => t.color === jokerColor
          );
          // Si jokers présents, ajoute un seul joker à la sélection
          if (jokerTiles.length > 0) {
        ...
        newState.center = newState.center.filter(
          (t) => !selectedTiles.includes(t)
        );
      ...
      const nextPlayerIndex =
        (currentPlayerIndex + 1) % newState.players.length;
    ...
    if (gameState.gamePhase === "tiling") {
      ...
      if (move.pass) {
        ...
        player.board.score = Math.max(
          1,
          player.board.score - tilesToDiscard.length
        );
        ...
        const allPassed = newState.players.every(
          (p) => !p.board.collectedTiles || p.board.collectedTiles.length === 0
        );
        if (allPassed) {
      ...
      const adjacentsBetweenFlowers = placed.filter(
        (pt) => adjacentFlowers.includes(pt.flower) && pt.pos === targetPos
      );
      points += adjacentsBetweenFlowers.length;

      // 3. Bonus rosette : si la fleur est complète après ce placement
      const tilesOnFlower = placed.filter(
        (pt) => pt.flower === targetFlower
      ).length;
      if (tilesOnFlower === 6) {
        // 6 positions par fleur
      ...
      const bonuses = checkBonusesOnPlacement(player.board.placedTiles || [], {
        flower: targetFlower,
        pos: targetPos,
      });

      if (bonuses.bonusTiles > 0) {
        // Le joueur doit choisir des tuiles bonus
        // Pour l'instant, on les ajoute automatiquement depuis le sac
        const availableTiles = [...newState.bag];
        const bonusTilesToAdd: Tile[] = [];

        for (
          let i = 0;
          i < bonuses.bonusTiles && availableTiles.length > 0;
          i++
        ) {
          const randomIndex = Math.floor(Math.random() * availableTiles.length);
          const bonusTile = availableTiles.splice(randomIndex, 1)[0];
          bonusTilesToAdd.push(bonusTile);
        }

        // Ajouter les tuiles bonus au sac du joueur
        if (!player.board.collectedTiles) {
          player.board.collectedTiles = [];
        }
        player.board.collectedTiles.push(...bonusTilesToAdd);

        // Retirer les tuiles du sac
        newState.bag = availableTiles;

        // Afficher un message ou déclencher une notification pour informer du bonus
        console.log(
          `Bonus obtenu! ${bonuses.bonusTiles} tuiles ajoutées au sac.`
        );
        if (bonuses.pillar) console.log("- Bonus Pilier: 1 tuile");
        if (bonuses.statue) console.log("- Bonus Statue: 2 tuiles");
        if (bonuses.window) console.log("- Bonus Fenêtre: 3 tuiles");
      }
      ...
      const anyoneHasTiles = newState.players.some(
        (p) => p.board.collectedTiles && p.board.collectedTiles.length > 0
      );
      ...
      if (!anyoneHasTiles) {
        ...
        while (tries < newState.players.length) {
          ...
          if (
            nextPlayer &&
            nextPlayer.board &&
            nextPlayer.board.collectedTiles &&
            nextPlayer.board.collectedTiles.length > 0
          ) {
            break;
          }
  ...
  allCostSpacesFilled(
    ...
    return flowers.every((flower) =>
      placedTiles.some(
        (tile) => tile.flower === flower && tile.pos === costPosition
      )
    );
  }
  ...
  canPlaceTiles(
    ...
    const isOccupied = placedTiles.some(
      (pt) => pt.flower === targetFlower && pt.pos === targetPos
    );
  ...
  calculateScore(gameState: GameState): number {
    ...
    return gameState.players.reduce(
      (total, player) => total + player.board.score,
      0
    );
  }
}
