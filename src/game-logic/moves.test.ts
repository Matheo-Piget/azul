...
describe("Game Moves", () => {
  ...
  describe("canPlaceTiles", () => {
    ...
    test("should return false if color is already on wall in the same row", () => {
      ...
      const blueIndex = player.board.wall[0].findIndex(
  ...
  describe("mustPlaceInFloorLine", () => {
    ...
    test("should return true if all valid pattern lines already have the color on wall", () => {
      ...
      for (let row = 0; row < 5; row++) {
        const blueIndex = player.board.wall[row].findIndex(
          (space) => space.color === "blue"
        );
    ...
    test("should return true if all pattern lines either have different colors or are full", () => {
      ...
      player.board.patternLines[1].tiles = [
        createTile("yellow"),
        createTile("yellow"),
      ];
      ...
      player.board.patternLines[3].tiles = [
        createTile("teal"),
        createTile("teal"),
        createTile("teal"),
        createTile("teal"),
      ...
      player.board.patternLines[4].tiles = [
        createTile("blue"),
        createTile("blue"),
        createTile("blue"),
        createTile("blue"),
  ...
  describe("Game Moves - edge cases & utils", () => {
    test("canSelectTiles returns false if factories array is empty and factoryId is not null", () => {
      const gameState: GameState = {
        players: [{ id: "p1", name: "Player p1", board: createPlayerBoard() }],
        factories: [],
        center: [],
        bag: [],
        discardPile: [],
    ...
    test("canPlaceTiles returns false if pattern line is full even if color matches", () => {
      ...
      // line 2 has 3 spaces, already full
      const gameState: GameState = {
        players: [player],
        factories: [],
        center: [],
    ...
    test("mustPlaceInFloorLine returns true if all pattern lines are full or blocked", () => {
      const player = createPlayer("p1");
      player.board.patternLines.forEach((line, i) => {
        line.color = "blue";
        line.tiles = Array(i + 1)
          .fill(null)
          .map(() => createTile("blue"));
      });
      const gameState: GameState = {
